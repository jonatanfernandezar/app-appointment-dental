...
export const deleteUser = async (req, res) => {
    ...
    // Verificar si el admin intenta borrarse a sí mismo
    if (userIdToDelete === loggedInUserId.toString()) {
        return res.status(403).json({ message: 'No puedes eliminar tu propia cuenta de admin.' });
    }

    try {
        // Verificar si el usuario a borrar es un admin o super admin
        const [userToDelete] = await pool.query('SELECT * FROM users WHERE id = ?', [userIdToDelete]);
        
        if (userToDelete.length === 0) {
            return res.status(404).json({ message: 'Usuario no encontrado.' });
        }

        if (userToDelete[0].role === 'admin' || userToDelete[0].is_super_admin === 1) {
            return res.status(403).json({ message: 'No puedes eliminar un usuario con rol de admin o super admin.' });
        }
